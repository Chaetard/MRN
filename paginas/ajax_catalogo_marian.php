...
$sql = 'SELECT id_empresa, nombre FROM paqueteria';
$stmt = $conn->query($sql);
$rows = $stmt->fetchAll();
if (empty($rows)) {
    ...
    <script>
        function showUser(str) {
            if (str == "") {
                ...
                xmlhttp.open("GET", "get_relacionada.php?q=" + str, true);
                xmlhttp.send();
            }
        }
    </script>
...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="content-wrapper mt-4">
            <fieldset>
                <legend class="text-primary">Paqueterias y sus envios</legend>
                <form id="formulario1" class="mt-3">
                    <div class="mb-3">
                        <label for="Combodepartamento" class="form-label">Paqueterias:</label>
                        <select name="Combodepartamento" id="Combodepartamento" class="form-select" onchange="showUser(this.value);">
                            <option value="0">-- Selecciona una paqueteria --</option>
                            <?php
                            foreach ($rows as $row) {
                    ...
                    <div class="mb-3">
                        <label for="ComboEmpleados" class="form-label">Envios de las paqueterias:</label>
                        <div id="txtHint" class="border p-3 rounded bg-light">Envios seleccionados</div>
                    </div>
                </form>
            </fieldset>
        </div>
